  ...
  return (
    <li>
      <Link href={url} className="flex justify-center group">
        <div className={`flex flex-col w-full sm:ml-0  sm:items-center relative ${ index % 2 != 0 ? 'sm:flex-row-reverse':'sm:flex-row'}` }>
          <img
            src={image.src}
            alt={title}
            width="214"
            ...
            loading="lazy"
          />
          <div className={`relative translate-x-12 sm:translate-x-0 -translate-y-12 -mb-12 gap-4 sm:gap-0 sm:mb-0 sm:translate-y-0 sm:absolute bg-white w-[80%] flex flex-col lg:flex-row sm:items-end py-4 px-4 rounded-[10px] rounded-bl-none border border-[#62CFF4] group-hover:bg-[#62CFF4] transition-colors duration-200 ease-in-out ${index % 2 !=0 ? 'sm:left-0' : 'sm:right-0'}`}>
            <div>
